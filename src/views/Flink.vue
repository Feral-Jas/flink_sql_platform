    ...
    <v-app id="inspire">
        <v-app-bar app color="white" flat>
            <v-container class="py-0 fill-height">
                ...
                <v-btn class="mr-2" elevation="1" v-for="link in links" :key="link" :color="links_color[link]" @click="action(link)">{{
                    link
                }}</v-btn>
                <v-snackbar top color="blue-grey" timeout="3000" v-model="snackbar">
                    {{ snackbarText }}
                </v-snackbar>
                <v-spacer></v-spacer>
                <v-dialog v-model="dialog" width="800px">
                    <template v-slot:activator="{ on, attrs }">
                        <v-btn class="mr-3" elevation="1" color="success" v-bind="attrs" v-on="on" @click="exportData()">导入导出</v-btn>
                    </template>
                    <v-card>
                ...
                <v-responsive max-width="260">
                    <v-text-field dense flat hide-details rounded solo-inverted v-model="describe" @change="assign(val)"></v-text-field>
                </v-responsive>
            </v-container>
        </v-app-bar>
...
export default class Flink extends Vue {
    ...
    action(link: string) {
        switch (link) {
            ...
            case '部署':
                console.log('deploy');
                this.snackbar = true;
                Vue.axios.post('http://localhost:3301/job', this.graphDataString).then((res) => {
    ...
    exportData() {
        const nodeDataArray = ((this.$refs.flowchart as Flowchart).diagram.model as GraphLinksModel).nodeDataArray;
        const linkDataArray = ((this.$refs.flowchart as Flowchart).diagram.model as GraphLinksModel).linkDataArray;
    ...
    importData() {
        ...
        ((this.$refs.flowchart as Flowchart).diagram.model as GraphLinksModel).nodeDataArray = this.graphData.nodeDataArray;
        ((this.$refs.flowchart as Flowchart).diagram.model as GraphLinksModel).linkDataArray = this.graphData.linkDataArray;
